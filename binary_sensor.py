...
def ensure_ccw_winding(polygon):
...
def offset_polygon(polygon, offset_meters):
    """Offsets a polygon by a given number of meters (approximate)."""
    if offset_meters == 0:
        return polygon
...
def load_polygons_from_geojson(file_path):
    try:
        ...
        for idx, feat in enumerate(features):
            ...
            outs = _coords_to_rings(geometry)
            for r_i, ring in enumerate(outs):
        ...
        _LOGGER.error("Error loading polygon(s) from %s: %s", file_path, e)
        return [], []
...
def _precompute_edges(polygon):
    n = len(polygon)
    return [ (polygon[i], polygon[(i+1) % n]) for i in range(n) ]

def _point_in_polygon_fast(point, polygon, edges=None, bbox=None):
    lon, lat = point
    if bbox:
        x1, y1, x2, y2 = bbox
...
def _point_segment_distance_m(lon, lat, a, b):
...
def _min_distance_to_edges_m(lon, lat, edges):
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities):
    ...
    tolerance = entry.options.get("tolerance", entry.data.get("tolerance", 0))
    ...
    for i, ring in enumerate(rings):
        ...
        entities.append(PolyZoneBinarySensor(
            f"{name} - {display}", ring, device_tracker_entity_id, entry,
            f"{base_id}_exact", label_exact, invert
        ))
        ...
        if tolerance and tolerance > 0:
            ...
            if offset_poly:
                entities.append(PolyZoneBinarySensor(
                    f"{name} - {display}", offset_poly, device_tracker_entity_id, entry,
                    f"{base_id}_tolerance", label_tol, invert, diagnostic=True
                ))
...
class PolyZoneBinarySensor(BinarySensorEntity):
    ...
    def __init__(self, name: str, polygon: list, device_tracker_entity_id: str, entry: ConfigEntry,
                 id_suffix: str, entity_name: str, invert: bool, diagnostic: bool=False):
    ...
    async def async_added_to_hass(self):
        self.async_on_remove(
            async_track_state_change_event(self.hass, self._device_tracker_entity_id, self._handle_state_change)
        )
    ...
    def _update_state(self, state):
        ...
        if self._latitude is not None and self._longitude is not None:
            inside = _point_in_polygon_fast((self._longitude, self._latitude), self._polygon, self._edges, self._bbox)
            now_on = (not inside) if self._invert else inside
            self._is_on = now_on
            self._distance_m = _min_distance_to_edges_m(self._longitude, self._latitude, self._edges)
        else:
            self._is_on = False
            self._distance_m = None
